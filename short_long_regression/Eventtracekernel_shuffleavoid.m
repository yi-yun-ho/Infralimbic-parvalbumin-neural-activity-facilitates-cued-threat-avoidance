function [b,rsq_train] = Eventtracekernel(filepath)
%UNTITLED6 Summary of this function goes here
%   Detailed explanation goes here

%20200330- comments out 185-190- use all data for calculating b instead of
%90% of data (no test train split)
% "To measure each variable’s unique contribution to the model, we created 
% reduced models in which we shuffled the regressor set corresponding to a 
% particular variable. The resulting loss of predictive power (?R2) relative 
% to the full model provides a lower bound for the amount of unique information 
% (non-redundant with any other model variable) that a given variable contributed to the model"
% -Simon Musall
...
%-1 sec to 2 sec -avoidance
 ...
 %shuffle this part of X
 X(1:size(X,1),7*Dfr+1:10*Dfr)=X(randperm(size(X,1))',7*Dfr+1:10*Dfr);
...
b=regress(Ys,Xs);
rsq_train=rsquare_function(Ys,Xs,b);
end
